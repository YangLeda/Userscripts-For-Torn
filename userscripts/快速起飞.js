...
(function () {
  ...
  const defaultDestination = "mexico"; // Modify here to change default destination.
  ...
  const destinationTimer = setInterval(() => {
    if (tabElementId === "") {
      ...
      if (tab2 && tab2.style.display !== "hidden") {
        console.log("QuickTravel: found tab4-2");
        tabElementId = "tab4-2";
      } else if (tab3 && tab3.style.display !== "hidden") {
    ...
    const targetElement = document.querySelector("div#" + tabElementId).querySelector("div.raceway." + defaultDestination);
    if (targetElement) {
